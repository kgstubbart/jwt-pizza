...
async function basicInit(page: Page) {
	...
	await page.route("*/**/api/auth", async (route) => {
		...
		const ok =
			user &&
			allowedPasswords[loginReq.email] &&
			allowedPasswords[loginReq.email].includes(loginReq.password);
		if (!ok) {
	...
	await page.route(/\/api\/franchise(\?.*)?$/, async (route) => {
		...
		if (method === "POST" || method === "PUT") {
			...
			await route.fulfill({ json: { franchise: created } });
			return;
		}
		await route.continue();
	});
	...
	await page.route(/\/api\/franchise\/\d+\/store$/, async (route) => {
		expect(["POST", "PUT"]).toContain(route.request().method());
		const body = route.request().postDataJSON();
		const created = { id: 77, ...body };
		await route.fulfill({ json: { store: created } });
	});
	...
	await page.route("*/**/api/user", async (route) => {
		...
		if (method === "POST" || method === "PUT") {
			...
			const created: any = {
				id: "100",
				name: body.name || body.fullName || "New User",
				email: body.email,
				roles: [{ role: Role.Diner }],
			};
